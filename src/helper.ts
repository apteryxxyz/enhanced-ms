...
export function pluralize(word: string, count: number): string {
  ...
  const measurement = Object.values(MEASUREMENTS).find(
    (m) => m.long === word || m.short === word || m.plural === word
  );
  ...
  if (measurement && count === 1) return measurement.long;
  // if count is greater than one use plural
  if (measurement && count > 1) return measurement.plural;
